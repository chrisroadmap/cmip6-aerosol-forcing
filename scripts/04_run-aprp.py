...
def aerchemmip():
    # I think there is only one ensemble member, and baseline and future should be the
    # same length.
    base_run = list(runs_histsst_piaer[model])[0]
    first_base = runs_histsst_piaer[model][base_run]['first']
    last_base = runs_histsst_piaer[model][base_run]['last']

    pert_run = list(runs_histsst_piaer[model])[0]
    outdir = f'../output/{model}/{pert_run}'
    os.makedirs(f'{outdir}/gridded', exist_ok=True)
    os.makedirs(f'{outdir}/mean', exist_ok=True)
    first_pert = runs_piclim_histaer[model][pert_run]['first']
    ...
    for year in tqdm(range(first_pert, last_pert+1), desc='Model years', leave=False):
        clt_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/clt_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsdt_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsdt_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsus_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsus_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsds_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsds_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsdscs_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsdscs_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsut_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsut_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsutcs_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsutcs_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rlut_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rlut_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rlutcs_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rlutcs_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")
        rsuscs_pert = iris.load_cube(f"{datadir}{model}/{pert_run}/histSST-piAer/rsuscs_Amon_{model}_histSST-piAer_{pert_run}_{year}.nc")

        clt_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/clt_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsdt_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsdt_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsus_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsus_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsds_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsds_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsdscs_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsdscs_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsut_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsut_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsutcs_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsutcs_Amon_{model}_histSST_{base_run}_{year}.nc")
        rlut_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rlut_Amon_{model}_histSST_{base_run}_{year}.nc")
        rlutcs_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rlutcs_Amon_{model}_histSST_{base_run}_{year}.nc")
        rsuscs_base = iris.load_cube(f"{datadir}{model}/{base_run}/histSST/rsuscs_Amon_{model}_histSST_{base_run}_{year}.nc")
