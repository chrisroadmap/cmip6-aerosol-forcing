...
def rfmip():
    for run in tqdm(runs_piclim_control[model][0:1], desc='base run', leave=False):
        outdir = f"../data/yearly/{model}/{run}/piClim-control/"
        os.makedirs(outdir, exist_ok=True)
        for var in tqdm(varlist, desc="variables", leave=False):
            for file in glob.glob(f"{datadir}/{var}_Amon_{model}_piClim-control_{run}_*.nc"):
    ...
    for run in tqdm(runs_piclim_histaer[model], desc='pert run', leave=False):
        ...
        os.makedirs(outdir, exist_ok=True)
        for var in tqdm(varlist, desc="variables", leave=False):
            for file in glob.glob(f"{datadir}/{var}_Amon_{model}_piClim-histaer_{run}_*.nc"):
                pert = iris.load_cube(file)
...
def aerchemmip():
    for run in tqdm(runs_histsst_piaer[model][0:1], desc='base run', leave=False):
        outdir = f"../data/yearly/{model}/{run}/histSST-piAer/"
        os.makedirs(outdir, exist_ok=True)
        for var in tqdm(varlist, desc="variables", leave=False):
            for file in glob.glob(f"{datadir}/{var}_Amon_{model}_histSST-piAer_{run}_*.nc"):
                base = iris.load_cube(file)
                iris.coord_categorisation.add_year(base, 'time')
                years = np.unique(base.coord("year").points)
                for year in years:
